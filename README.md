...
```bash
uv run scripts/compute_norm_stats.py --config-name real_xarm6
```

Now we can kick off training with the following command (the `--overwrite` flag is used to overwrite existing checkpoints if you rerun fine-tuning with the same config):

```bash
XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 uv run scripts/train.py real_xarm6 --exp-name=my_experiment --overwrite
```
...
```bash
uv run scripts/serve_policy.py policy:checkpoint --policy.config=real_xarm6 --policy.dir=checkpoints/pi0_fast_libero/my_experiment/20000
```
